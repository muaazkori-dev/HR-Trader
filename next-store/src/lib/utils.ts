export const getProductImageUrl = (
  ...
  if (trimmed.startsWith('https://') || trimmed.startsWith('http://')) {
    if (trimmed.startsWith('http://xarwwlbbaevclyljkvzt.supabase.co')) {
      ...
      if (secureUrl.includes('/storage/v1/object/public/')) {
        return secureUrl.replace('/storage/v1/object/public/', '/storage/v1/render/image/public/') + `?width=${width}&quality=${quality}&resize=${resize}`;
      }
      return secureUrl;
    }
    return trimmed;
  ...
  // Fallback to Supabase Cloud Storage Render endpoint with automatic optimization and no crop
  return `https://xarwwlbbaevclyljkvzt.supabase.co/storage/v1/render/image/public/product-images/products/${filename}?width=${width}&quality=${quality}&resize=${resize}`;
};
